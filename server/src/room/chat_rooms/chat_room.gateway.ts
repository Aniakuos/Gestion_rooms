...
export class AppGateway {
	...
	{	
		...
		const newObj = this.chatroomservice.add_user_to_room(infos);
        if (newObj)
            this.server.to(infos.room_id).emit("Joinned", {"user_role": (await newObj).user_role, "user_id": (await newObj).user_id}); //!!!!!!!add user Abdellah
    }
